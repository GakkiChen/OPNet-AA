...
class Cater6TracksForObjectsTrainingDataset(CaterAbstract6TracksForObjectsDataset):
    def __init__(self, predictions_dir: str, label_dir: str, mask_annotations_path: str, track_vector_path: str):
        super().__init__(predictions_dir, label_dir)
        self.mask_annotations_path: str = mask_annotations_path
        self.track_vector_path: str = track_vector_path
    ...
    def _init_dataset_if_not_initiated(self) -> None:
        ...
        if len(self.videos_names) == 0:
            ...
            # init track_matrix input
            track_dict = json.load(open(self.track_vector_path))
            for vname, track_vec in track_dict.items():
                track_mat = torch.ones((300, self.max_objects))
                if track_vec[0] != 999:
                    # when no exception exists
                    for idx, val in enumerate(track_vec):
                        if val < 15:
                            track_mat[idx, val] *= 5

#                     with torch.no_grad():
#                         track_mat = F.softmax(track_mat, dim=1)
                self.track_frames[vname] = track_mat
    ...
    def __getitem__(self, idx: int) -> Tuple[Tuple[torch.tensor, torch.tensor], Tuple[torch.tensor, torch.tensor], str]:
        ...
        # get track_matrix
        tensor_track_mat = self.track_frames[video_name]
        ...
        return (tensor_boxes, tensor_index_to_track, tensor_track_mat), (tensor_labels, tensor_frame_mask), video_name


class Cater6TracksForObjectsInferenceDataset(CaterAbstract6TracksForObjectsDataset):
    def __init__(self, predictions_dir: str, label_dir: str, track_vector_path: str):
        super().__init__(predictions_dir, label_dir)
        self.track_vector_path: str = track_vector_path
        self.track_frames = {}
...
#                         track_mat = F.softmax(track_mat, dim=1)
    ...
    def __getitem__(self, idx: int) -> Tuple[Tuple[torch.tensor, torch.tensor], Tuple[torch.tensor, torch.tensor], str]:
        ...
        return (tensor_boxes, tensor_index_to_track, tensor_track_mat), (tensor_labels, empty_tensor), video_name
