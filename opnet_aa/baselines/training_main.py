...
def inference_and_iou_comp(model_name: str, model: nn.Module, compute_device: torch.device, data_loader: data.DataLoader, dataset_length: int,
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(data_loader):
            ...
            boxes, track_mat = boxes.to(compute_device), track_mat.to(compute_device)
            labels, mask = labels.to(compute_device), mask.to(compute_device)

            if model_name in DOUBLE_OUTPUT_MODELS:
                output, index_to_track_prediction = model(boxes, track_mat)

            else:
                output = model(boxes, track_mat)
...
def training_main(model_name: str, train_config: Dict[str, Any], model_config: Dict[str, int]):
    ...
    train_track_vector_file = train_config["train_track_vector_file"]
    ...
    dev_track_vector_file = train_config["dev_track_vector_file"]

    train_dataset: data.Dataset = DatasetsFactory.get_training_dataset(model_name, train_samples_dir, train_labels_dir, train_containment_file, train_track_vector_file)
    dev_dataset: data.Dataset = DatasetsFactory.get_training_dataset(model_name, dev_samples_dir, dev_labels_dir, dev_containment_file, dev_track_vector_file)
    ...
    # create dictionary to store train and dev loss
    train_loss_dict = {'avg_loss':[], 'pred_loss':[], 'cons_loss':[]}
    train_loss_epoch_dict = {'loss':[], 'miou':[], 'c_miou':[]}
    dev_loss_epoch_dict = {'loss':[], 'miou':[], 'c_miou':[]}
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, sample in enumerate(training_loader, 1):
            ...
            boxes, _, track_mat = x
            labels, mask = y
            boxes, track_mat = boxes.to(device), track_mat.to(device)
            ...
            if batch_idx % print_batch_step == 0:
                ...
                # store the loss
                train_loss_dict['avg_loss'].append(average_running_loss)
                train_loss_dict['pred_loss'].append(average_pred_loss)
                train_loss_dict['cons_loss'].append(average_consist_loss)
        ...
        # store the loss
        train_loss_epoch_dict['loss'].append(train_loss)
        train_loss_epoch_dict['miou'].append(train_miou)
        train_loss_epoch_dict['c_miou'].append(train_containment_miou)
        dev_loss_epoch_dict['loss'].append(dev_loss)
        dev_loss_epoch_dict['miou'].append(dev_miou)
        dev_loss_epoch_dict['c_miou'].append(dev_containment_miou)
    ...
    # save loss log
    with open(os.path.join(checkpoints_path, 'train_loss.json'), 'w') as fp:
        json.dump(train_loss_dict, fp)
    with open(os.path.join(checkpoints_path, 'train_loss_epoch.json'), 'w') as fp:
        json.dump(train_loss_epoch_dict, fp)
    with open(os.path.join(checkpoints_path, 'dev_loss_epoch.json'), 'w') as fp:
        json.dump(dev_loss_epoch_dict, fp)
