...
def reasoning_inference_main(model_name: str, results_dir: str, inference_config_path: str, model_config_path: str):
    ...
    samples_dir = config["sample_dir"]
    labels_dir = config["labels_dir"]
    track_mat_file = config["track_vector_file"]
    batch_size = int(config["batch_size"])
    num_workers = int(config["num_workers"])
    model_path = config["model_path"]
    device = torch.device(config["device"])

    dataset: data.Dataset = DatasetsFactory.get_inference_dataset(model_name, samples_dir, labels_dir, track_mat_file)
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(data_loader):
            ...
            boxes, index_to_track_labels, track_mat = x
            labels, _ = y
